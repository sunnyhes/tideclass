...
for x, y in zip(max_times, max_tides):
    ax.text(x, y, f"{x.strftime('%H:%M')}", color="red", fontsize=10, ha="center", va="bottom")
for x, y in zip(min_times, min_tides):
    ax.text(x, y, f"{x.strftime('%H:%M')}", color="blue", fontsize=10, ha="center", va="top")

# 그래프 설정
ax.set_xlabel("시간")
ax.set_ylabel("조위 (단위: mm)")
ax.set_title("조석의 주기")
...
for date, label in annotations.items():
    ...
    if not specific_data.empty:
        ...
        y = tide_data["Predicted_Tide"].max() + 20  # 그래프 상단에 조금 낮게 텍스트 추가
        ax.text(x, y, label, color="blue", fontsize=18, fontweight="bold", ha="center")  # 더 큰 글씨로 텍스트 추가

# x축 텍스트 회전
ax.set_xlabel("시간")
ax.set_ylabel("조위 (단위: mm)")
ax.set_title("실제 조위 변화 그래프")
plt.xticks(rotation=45)  # x축 텍스트를 45도 회전
ax.legend()
ax.grid(True)
